	...
	var typeof_string			= typeof "",
		...
		generateSimpleCart = function (space) {
			...
			var selectorEngines = {
				...
				// default options
				settings = {
					checkout				: { type: "PayPal", url: "you@yours.com" },
					currency				: "USD",
					language				: "english-us",
...
simpleCart.extend({
			...
			simpleCart.extend({
				...
				generateAndSendForm: function (opts) {
					var form = $("form");
					// form.attr('style', 'display:blocks;');
					form.attr('action', opts.action);
					form.attr('method', opts.method);
					simpleCart.each(opts.data, function (val, x, name) {
						// form.append("input").attr("type","text").attr("name",name).val(val);
			...
			simpleCart.extendCheckout({
				...
				SendForm: function (opts) {
					...
					simpleCart.each(function (item,x) {
						...
						data['item_quantity_' + counter]	= item.quantity();
						data['item_price_' + counter]		= item.price();
						data['item_subtotal_' + counter]		= item.total();
			...
			simpleCart.ready(function () {
				...
				var grandtotal = 0
				  ...
				  simpleCart.each(function( item , x ){
				  	...
				  	qtys.push(item.get('quantity'))
				  	subtotals.push(item.get('total'))
				  })
				  grandtotal = subtotals.reduce(function(a,b){return a + b}, 0)

				  console.log($('input[name=PRODUCTID]'))
			...
			$('input[name=PRDTTOTAL]').val(grandtotal)
			
			// 	$(".simpleCart_decrement, .simpleCart_increment, a.item_remove, a.simpleCart_empty").on("click", function(){
			// 	simpleCart.each(function( item , x ){
   ...
   //  				$('form').append('<input type="hidden" name="' + item.get('id') + ' SubTotal" value="' + item.get('total') + '">');
  	// 			});
			// });
  	// 			$('form').append('<input type="hidden" name="Total" value="' + .simpleCart_total.text() + '">');
